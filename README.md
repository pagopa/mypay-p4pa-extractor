...
| payment_notification                         | `/db/mypay/payment-notification/payment-notification.sql` | <li>ipaCode: organization IPA code (required)<br><li>iud: payment IUD filter on `mygov_flusso_import.cod_iud` (nullable)<br><li>iuv: payment IUV filter on `mygov_flusso_import.cod_rp_silinviarp_id_univoco_versamento` (nullable)<br><li>createdFrom: inclusive creation timestamp applied to `mygov_flusso_import.dt_creazione` (nullable)<br><li>createdTo: inclusive creation timestamp applied to `mygov_flusso_import.dt_creazione` (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `PAYMENT_NOTIFICATION` extractor from `mygov_flusso_import`, joined to `mygov_ente`, scoped to one organization and supporting optional IUD, IUV, and creation timestamp filters. Creation date filters are skipped when the corresponding parameter is null. Results are ordered by `dt_creazione`. |
