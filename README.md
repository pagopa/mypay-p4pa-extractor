...
|----------------------------------------------| ------------------------- | ------------------------------------------------ | ----------------------------- |
| organization                                 | `/db/mypay/organization/organization.sql` | <li>ipaCodes: organization IPA codes<br><li>skipDateFromFilter: `true` to skip the modification lower-bound filter<br><li>dateFrom: extraction start timestamp (nullable; falls back to `lastExtractionDate` when no explicit start is supplied)<br><li>skipDateToFilter: `true` to skip the modification upper-bound filter<br><li>dateTo: extraction end timestamp (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | Organization entity extractor |
...
| debt_positions_paid                          | `/db/mypay/debt-positions-paid/debt-positions-paid.sql` | <li>codIpaEnte: organization IPA code (required)<br><li>iuvs: IUV filter on `mygov_dovuto_elaborato.cod_rp_silinviarp_id_univoco_versamento` (nullable list)<br><li>skipIuvsFilter: `true` to skip the `iuvs` filter<br><li>dateFrom: extraction start timestamp applied to `mygov_dovuto_elaborato.dt_creazione` (nullable; falls back to `lastExtractionDate` when no explicit start is supplied)<br><li>skipCreatedFromFilter: `true` to skip the lower-bound filter<br><li>dateTo: extraction end timestamp applied to `mygov_dovuto_elaborato.dt_creazione` (nullable)<br><li>skipCreatedToFilter: `true` to skip the upper-bound filter<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `DEBT_POSITIONS_PAID` extractor for completed debt positions from `mygov_dovuto_elaborato`, joined to `mygov_flusso`, `mygov_ente`, `mygov_anagrafica_stato`, and `mygov_receipt`, with an optional IUV filter and creation date interval. |
### FESP
...
| payment_notification          | `/db/mypivot/payment-notification/payment-notification.sql` | <li>ipaCode: organization IPA code (required)<br><li>iudsEmpty: `true` to skip the IUD filter<br><li>iuds: payment IUDs to include (nullable list)<br><li>iuvsEmpty: `true` to skip the IUV filter<br><li>iuvs: payment IUVs to include (nullable list)<br><li>skipCreatedFromFilter: `true` to skip the creation lower-bound filter<br><li>dateFrom: creation timestamp lower bound (nullable; falls back to `lastExtractionDate` when no explicit start is supplied)<br><li>skipCreatedToFilter: `true` to skip the creation upper-bound filter<br><li>dateTo: creation timestamp upper bound (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `PAYMENT_NOTIFICATION` extractor from MyPivot `mygov_flusso_import`, joined to `mygov_ente`, with optional IUD, IUV, and creation-timestamp filters. Results are ordered by creation timestamp. |
