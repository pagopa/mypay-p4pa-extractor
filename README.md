...
| `PAYMENTS_REPORTING_BASE_DIRECTORY` | Existing MyPay directory used to resolve relative XML file names returned by the FESP payments-reporting query as `<base-directory>/<COD_PA_ENTE>/<file-name>`. Each XML is copied to the extractor temporary working directory before it is added to the export ZIP. Absolute paths are used unchanged. | `TMP_FOLDER` |
...
| `EXPORT_PAGE_SIZE_PAYMENTS_REPORTING`                 | Maximum number of XML reporting files retrieved from FESP and copied from MyPay to extractor staging at a time | `1000` |
...
| debt_positions_paid                          | `/db/mypay/debt-positions-paid/debt-positions-paid.sql` | <li>codIpaEnte: organization IPA code (required)<br><li>iuds: IUD filter on `mygov_dovuto_elaborato.cod_iud` (nullable list)<br><li>iudsEmpty: `true` to skip the `iuds` filter<br><li>iuvs: IUV filter on `mygov_dovuto_elaborato.cod_rp_silinviarp_id_univoco_versamento` (nullable list)<br><li>iuvsEmpty: `true` to skip the `iuvs` filter<br><li>createdFrom: extraction start timestamp applied to `mygov_dovuto_elaborato.dt_creazione` (nullable)<br><li>skipCreatedFromFilter: `true` to skip the `createdFrom` filter<br><li>createdTo: extraction end timestamp applied to `mygov_dovuto_elaborato.dt_creazione` (nullable)<br><li>skipCreatedToFilter: `true` to skip the `createdTo` filter<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `DEBT_POSITIONS_PAID` extractor for completed debt positions from `mygov_dovuto_elaborato`, joined to `mygov_flusso`, `mygov_ente`, `mygov_anagrafica_stato`, and `mygov_receipt`, with optional filters on paired IUD/IUV and creation date interval. |
### FESP

| PU Entity | Resource | Parameters | Description |
|---|---|---|---|
| payments_reporting | `/db/fesp/payments-reporting/payments-reporting.sql` | <li>ipaCode: organization IPA code (required)<br><li>dateFrom: inclusive modification timestamp lower bound (nullable)<br><li>skipDateFromFilter: `true` to skip the `dateFrom` filter<br><li>dateTo: inclusive modification timestamp upper bound (nullable)<br><li>skipDateToFilter: `true` to skip the `dateTo` filter<br><li>skipLogicalKeyFilter: `true` to skip the flow identifier filter<br><li>logicalKey: flow identifier (required for logical-key lookup)<br><li>limit: page size and maximum XML files per ZIP (> 0)<br><li>offset: page start (>= 0, nullable) | Retrieves XML reporting file names with status `OK` from FESP `mygov_flusso_rend_spc`. Each page is resolved from the configured MyPay directory, copied to extractor staging, then packaged in a separate ZIP without CSV generation or data transformation. ZIP names are prefixed with the broker IPA code and include the organization IPA code; multi-page exports add `part001`, `part002`, and so on. |
...
| payment_notification          | `/db/mypivot/payment-notification/payment-notification.sql` | <li>ipaCode: organization IPA code (required)<br><li>iudsEmpty: `true` to skip the IUD filter<br><li>iuds: payment IUDs to include (nullable list)<br><li>iuvsEmpty: `true` to skip the IUV filter<br><li>iuvs: payment IUVs to include (nullable list)<br><li>skipCreatedFromFilter: `true` to skip the creation lower-bound filter<br><li>createdFrom: creation timestamp lower bound (nullable)<br><li>skipCreatedToFilter: `true` to skip the creation upper-bound filter<br><li>createdTo: creation timestamp upper bound (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `PAYMENT_NOTIFICATION` extractor from MyPivot `mygov_flusso_import`, joined to `mygov_ente`, with optional IUD, IUV, and creation-timestamp filters. Results are ordered by creation timestamp. |
| treasury_csv_complete         | `/db/mypivot/treasury-csv-complete/treasury-csv-complete.sql` | <li>ipaCode: organization IPA code (required, resolved one at a time from the request ipaCodes)<br><li>skipAnnoBollettaFilter: `true` to skip the `annoBolletta` filter<br><li>annoBolletta: first component of the optional `<annoBolletta>\|<codBolletta>` logical key<br><li>skipCodBollettaFilter: `true` to skip the `codBolletta` filter<br><li>codBolletta: second component of the optional logical key<br><li>skipUpdatedFromFilter: `true` to skip the modification lower-bound filter<br><li>updatedFrom: modification timestamp lower bound (nullable)<br><li>skipUpdatedToFilter: `true` to skip the modification upper-bound filter<br><li>updatedTo: modification timestamp upper bound (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `TREASURY_CSV_COMPLETE` extractor for treasury flows from `mygov_flusso_tesoreria`, joined to `mygov_ente`; unavailable source fields `codEnteBT` and `codIstatEnte` are emitted as `NULL`. Incremental extraction uses exclusively `dt_ultima_modifica`. |
