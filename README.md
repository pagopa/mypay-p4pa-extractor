...
| assessments                   | `/db/mypivot/assessments/assessments-export-by-last-migration.sql` / `/db/mypivot/assessments/assessments-export-by-date-range.sql` | <li>codIpaEnte: organization IPA code<br><li>dataUltimaMigrazione: last migration timestamp (nullable when the date-range query is used)<br><li>dateFrom: extraction start timestamp (nullable)<br><li>dateTo: extraction end timestamp (nullable) | `ASSESSMENTS` extractor from `mygov_accertamento` joined to `mygov_accertamento_dettaglio`, selecting the SQL variant in the DAO based on whether both date bounds are provided. |
