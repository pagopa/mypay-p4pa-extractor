...
| treasury_csv_complete         | `/db/mypivot/treasury-csv-complete/treasury-csv-complete.sql` | <li>ipaCodes: organization IPA code list (required)<br><li>annoBolletta: first component of the optional `<annoBolletta>\|<codBolletta>` logical key<br><li>codBolletta: second component of the optional logical key<br><li>createdFrom/createdTo: inclusive interval on `mygov_flusso_tesoreria.dt_creazione` (nullable)<br><li>updatedFrom/updatedTo: inclusive interval on `mygov_flusso_tesoreria.dt_ultima_modifica` (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `TREASURY_CSV_COMPLETE` extractor for treasury flows from `mygov_flusso_tesoreria`, joined to `mygov_ente`; unavailable source fields `codEnteBT` and `codIstatEnte` are emitted as `NULL`. |
