...
### 📥 Input payload details (`filters.logicalKey`)

`filters.logicalKey` is optional and its format depends on `fileTypes`.

| MigrationFileType | `filters.logicalKey` format |
|---|---|
| `ORGANIZATIONS` | Not used |
| `ORG_SIL_SERVICES` | Not used |
| `DEBT_POSITIONS_TYPE` | Comma-separated list of `debtPositionTypeOrgCode` values (`code1,code2`) |
| `DEBT_POSITIONS_TYPE_ORG` | Comma-separated list of `debtPositionTypeOrgCode` values (`code1,code2`) |
| `DEBT_POSITIONS_TYPE_ORG_OPERATORS` | Two comma-separated lists separated by `\|`: `operatorFiscalCodes\|debtPositionTypeOrgCodes` |
| `DEBT_POSITIONS` | Two comma-separated lists separated by `\|`: `gpdIupds\|iuds` |

Rules:
* For a single-list format, values must be non-empty.
* For a paired format, the value must contain exactly one `|`, and both sides must be non-empty comma-separated lists.
...
| debt_positions_type_org_operators            | `/db/mypay/debt-positions-type-org-operators/debt-positions-type-org-operators.sql` | <li>ipaCode: organization IPA code<br><li>operatorFiscalCodesEmpty: `true` when no operator fiscal code filter is provided<br><li>operatorFiscalCodes: operator fiscal codes to include<br><li>debtPositionTypeOrgCodesEmpty: `true` when no debt position type org code filter is provided<br><li>debtPositionTypeOrgCodes: debt position type org codes to include<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `DEBT_POSITIONS_TYPE_ORG_OPERATORS` extractor scoped to one organization. Reads MyPay operator associations for debt position types and supports logical-key list filters with pagination. |
| debt_positions (open/updated)                | `/db/mypay/debt-positions/debt-positions-open.sql` | <li>codIpaEnte: organization IPA code (required)<br><li>gpdIupdsEmpty: `true` when no external-key filter is provided<br><li>gpdIupds: debt position external keys to include<br><li>iudsEmpty: `true` when no IUD filter is provided<br><li>iuds: debt position IUDs to include<br><li>updatedFrom: extraction start date at 00:00 applied to `mygov_dovuto.dt_ultima_modifica` (nullable)<br><li>updatedToExclusive: extraction end date +1 day at 00:00 applied to `mygov_dovuto.dt_ultima_modifica` (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `DEBT_POSITIONS` extractor for open or modified debt positions from `mygov_dovuto`, joined to `mygov_flusso` and `mygov_ente`, with optional logical-key list filters on `gpd_iupd` and `cod_iud`. |
