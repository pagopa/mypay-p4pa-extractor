...
| assessments                   | `/db/mypivot/assessments/assessments-export.sql` | <li>ipaCode: organization IPA code<br><li>skipOrgIpaCodesFilter: `true` to skip the organization IPA list filter<br><li>orgIpaCodes: organization IPA codes to include<br><li>assessmentCodes: assessment names to include (nullable list)<br><li>skipAssessmentCodesFilter: `true` to skip the `assessmentCodes` filter<br><li>dateFrom: extraction start timestamp (nullable)<br><li>skipDateFromFilter: `true` to skip the `dateFrom` filter<br><li>dateTo: extraction end timestamp (nullable)<br><li>skipDateToFilter: `true` to skip the `dateTo` filter<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `ASSESSMENTS` extractor from MyPivot. |
| assessments_registry         | `/db/mypivot/assessments-registry/assessments-registry-export.sql` | <li>ipaCode: organization IPA code<br><li>skipOrgIpaCodesFilter: `true` to skip the organization IPA list filter<br><li>orgIpaCodes: organization IPA codes to include<br><li>debtPositionTypeOrgCodes: debt position type organization codes to include (nullable list)<br><li>skipDebtPositionTypeOrgCodesFilter: `true` to skip the `debtPositionTypeOrgCodes` filter<br><li>dateFrom: extraction start timestamp (nullable)<br><li>skipDateFromFilter: `true` to skip the `dateFrom` filter<br><li>dateTo: extraction end timestamp (nullable)<br><li>skipDateToFilter: `true` to skip the `dateTo` filter<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `ASSESSMENTS_REGISTRY` extractor from MyPivot. |
