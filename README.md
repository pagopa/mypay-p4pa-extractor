...
| PU Entity | Resource | Parameters | Description |
|---|---|---|---|
| payments_reporting | `/db/mypay/payments-reporting/payments-reporting.sql` | <li>organizationId: organization IPA code (required)<br><li>lastExtractionDate: last extraction timestamp, used when `dateFrom` is absent (nullable)<br><li>dateFrom: inclusive modification timestamp lower bound (nullable)<br><li>dateTo: inclusive modification timestamp upper bound (nullable)<br><li>skipLogicalKeyFilter: `true` to skip the flow identifier filter<br><li>logicalKey: flow identifier (required for logical-key lookup) | Retrieves XML reporting files with status `OK` from FESP `mygov_flusso_rend_spc`. The result paths are packaged in a ZIP without CSV generation or data transformation. |
...
| assessments                   | `/db/mypivot/assessments/assessments-export.sql` | <li>ipaCode: organization IPA code<br><li>lastExtractionDate: last extraction timestamp used when a complete date range is not provided<br><li>skipLastExtractionDateFilter: `true` to skip the `lastExtractionDate` filter<br><li>assessmentCodes: assessment names to include (nullable list)<br><li>skipAssessmentCodesFilter: `true` to skip the `assessmentCodes` filter<br><li>dateFrom: extraction start timestamp (nullable)<br><li>skipDateFromFilter: `true` to skip the `dateFrom` filter<br><li>dateTo: extraction end timestamp (nullable)<br><li>skipDateToFilter: `true` to skip the `dateTo` filter<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `ASSESSMENTS` extractor from `mygov_accertamento` joined to `mygov_accertamento_dettaglio`. It optionally filters by assessment names, applies the inclusive `dateFrom`/`dateTo` interval when both bounds are provided, and otherwise applies the incremental last-extraction filter. Results are paginated with `LIMIT/OFFSET`. |
| assessments_registry         | `/db/mypivot/assessments-registry/assessments-registry-export.sql` | <li>ipaCode: organization IPA code<br><li>lastExtractionDate: last extraction timestamp used when `dateFrom` is not provided<br><li>skipLastExtractionDateFilter: `true` to skip the `lastExtractionDate` filter<br><li>debtPositionTypeOrgCodes: debt position type organization codes to include (nullable list)<br><li>skipDebtPositionTypeOrgCodesFilter: `true` to skip the `debtPositionTypeOrgCodes` filter<br><li>dateFrom: extraction start timestamp (nullable)<br><li>skipDateFromFilter: `true` to skip the `dateFrom` filter<br><li>dateTo: extraction end timestamp (nullable)<br><li>skipDateToFilter: `true` to skip the `dateTo` filter<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `ASSESSMENTS_REGISTRY` extractor from `mygov_anagrafica_uff_cap_acc` joined to `mygov_ente`. It supports optional debt position type organization code and modification-date filters and is paginated with `LIMIT/OFFSET`. |
