...
| assessments                   | `/db/mypivot/assessments/assessments-export.sql` | <li>codIpaEnte: organization IPA code<br><li>lastExtractionDate: last extraction timestamp used when a complete date range is not provided<br><li>dateFrom: extraction start timestamp (nullable)<br><li>dateTo: extraction end timestamp (nullable) | `ASSESSMENTS` extractor from `mygov_accertamento` joined to `mygov_accertamento_dettaglio`. It applies the inclusive `dateFrom`/`dateTo` interval when both bounds are provided; otherwise it applies the incremental last-extraction filter. |
