...
| debt_positions_type_org | `/db/mypay/debt-position-type-org/debt-position-type-org.sql` | <li>codIpaEnte: organization IPA code (nullable)<br><li>modifiedFrom: extraction start date at 00:00 (nullable)<br><li>modifiedToExclusive: extraction end date +1 day at 00:00 (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | DEBT_POSITIONS_TYPE_ORG extractor. Reads `mygov_ente_tipo_dovuto` joined to `mygov_ente`, keeps the lowest `mygov_ente_tipo_dovuto_id` for each `cod_tipo` using `ROW_NUMBER()`, and supports IPA, logical-key, incremental, and pagination filters. |
...
|debt_positions_type_org| `mypivot/debt-position-type-org/is-external.sql`| <li>codIpaEnte: organization IPA code | Check if `DEBT_POSITIONS_TYPE_ORG` is external for organization |
