...
| debt_positions_type                          | `/db/mypay/debt-position-type/debt-position-type.sql` | <li>brokerCf: intermediary fiscal code from `BROKER_CF` or `PA_IDENTIFICATIVO_INTERMEDIARIO_PA`<br><li>skipDebtPositionTypeCodesFilter: `true` when no debt position type code filter is provided<br><li>debtPositionTypeCodes: debt position type codes to include<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `DEBT_POSITIONS_TYPE` extractor filtered only by debt position type codes; its logical key does not contain organization IPA codes. |
| debt_positions_type_org                      | `/db/mypay/debt-position-type-org/debt-position-type-org.sql` | <li>ipaCode: organization IPA code<br><li>skipOrgIpaCodesFilter: `true` when no organization IPA list is provided<br><li>orgIpaCodes: organization IPA codes to include<br><li>skipDebtPositionTypeOrgCodesFilter: `true` when no debt position type org code filter is provided<br><li>debtPositionTypeOrgCodes: debt position type org codes to include<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `DEBT_POSITIONS_TYPE_ORG` extractor using the `orgIpaCodes|debtPositionTypeOrgCodes` logical-key format. |
| debt_positions_type_org_operators            | `/db/mypay/debt-positions-type-org-operators/debt-positions-type-org-operators.sql` | <li>ipaCode: organization IPA code<br><li>skipLogicalKeyFilter: `true` when either logical-key component is absent<br><li>logicalKeys: paired `(debtPositionTypeOrgCode, operatorFiscalCode)` values from the logical key<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `DEBT_POSITIONS_TYPE_ORG_OPERATORS` extractor scoped to one organization. |
| debt_positions (open/updated)                | `/db/mypay/debt-positions/debt-positions-open.sql` | <li>codIpaEnte: organization IPA code (required)<br><li>skipOrgIpaCodesFilter: `true` to skip the organization IPA list filter<br><li>orgIpaCodes: organization IPA codes to include<br><li>skipCodIuvFilter: `true` to skip the `iuvs` filter<br><li>iuvs: debt position IUV filter on `mygov_dovuto.cod_iuv` (nullable list)<br><li>skipDateFromFilter: `true` to skip the `dateFrom` filter<br><li>dateFrom: extraction start date at 00:00 applied to `mygov_dovuto.dt_ultima_modifica` (nullable)<br><li>skipDateToExclusiveFilter: `true` to skip the `dateToExclusive` filter<br><li>dateToExclusive: extraction end date +1 day at 00:00 applied to `mygov_dovuto.dt_ultima_modifica` (nullable)<br><li>skipGpdEnabledFilter: `true` to skip the GPD-only filter<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `DEBT_POSITIONS` extractor for open or modified debt positions. |
...
| PU Entity | Resource | Parameters | Description |
|---|---|---|---|
| payments_reporting | `/db/fesp/payments-reporting/payments-reporting.sql` | <li>ipaCode: organization IPA code (required)<br><li>skipOrgIpaCodesFilter: `true` to skip the organization IPA list filter<br><li>orgIpaCodes: organization IPA codes to include<br><li>dateFrom: inclusive modification timestamp lower bound (nullable)<br><li>skipDateFromFilter: `true` to skip the `dateFrom` filter<br><li>dateTo: inclusive modification timestamp upper bound (nullable)<br><li>skipDateToFilter: `true` to skip the `dateTo` filter<br><li>skipLogicalKeyFilter: `true` to skip the flow identifier filter<br><li>logicalKeys: flow identifiers to include (nullable list, and combinable with date filters)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | Retrieves XML reporting files with status `OK` from FESP. |
...
| payment_notification          | `/db/mypivot/payment-notification/payment-notification.sql` | <li>ipaCode: organization IPA code (required)<br><li>skipLogicalKeyFilter: `true` when either logical-key component is absent<br><li>logicalKeys: paired `(iud, iuv)` values from the logical key<br><li>skipCreatedFromFilter: `true` to skip the creation lower-bound filter<br><li>createdFrom: creation timestamp lower bound (nullable)<br><li>skipCreatedToFilter: `true` to skip the creation upper-bound filter<br><li>createdTo: creation timestamp upper bound (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `PAYMENT_NOTIFICATION` extractor from MyPivot. |
| treasury_csv_complete         | `/db/mypivot/treasury-csv-complete/treasury-csv-complete.sql` | <li>ipaCode: organization IPA code (required, resolved one at a time from the request ipaCodes)<br><li>skipBollettaLogicalKeyFilter: `true` when either bill logical-key component is absent<br><li>bollettaLogicalKeys: paired `(annoBolletta, codBolletta)` values from the logical key<br><li>skipUpdatedFromFilter: `true` to skip the modification lower-bound filter<br><li>updatedFrom: modification timestamp lower bound (nullable)<br><li>skipUpdatedToFilter: `true` to skip the modification upper-bound filter<br><li>updatedTo: modification timestamp upper bound (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `TREASURY_CSV_COMPLETE` extractor for treasury flows. |
