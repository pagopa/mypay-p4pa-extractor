...
| debt_positions_type_org | `/db/mypay/debt-position-type-org/debt-position-type-org.sql` | <li>ipaCode: organization IPA code<br><li>logicalKeysEmpty: `true` when no logical-key filter is provided<br><li>logicalKeys: debt position type logical keys to include<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `DEBT_POSITIONS_TYPE_ORG` extractor scoped to one organization. Reads `mygov_ente_tipo_dovuto` joined to `mygov_ente` and supports logical-key and pagination filters. |
