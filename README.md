...
| treasury_csv_complete         | `/db/mypivot/treasury-csv-complete/treasury-csv-complete.sql` | <li>ipaCode: organization IPA code (required, resolved one at a time from the request ipaCodes)<br><li>annoBolletta: first component of the optional `<annoBolletta>\|<codBolletta>` logical key<br><li>codBolletta: second component of the optional logical key<br><li>dateFrom/dateTo: inclusive interval on `mygov_flusso_tesoreria.dt_ultima_modifica` (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `TREASURY_CSV_COMPLETE` extractor for treasury flows from `mygov_flusso_tesoreria`, joined to `mygov_ente`; unavailable source fields `codEnteBT` and `codIstatEnte` are emitted as `NULL`. Incremental extraction uses exclusively `dt_ultima_modifica`. |
