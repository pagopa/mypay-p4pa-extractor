...
| assessments_registry         | `/db/mypivot/assessments-registry/assessments-registry-export.sql` | <li>ipaCode: organization IPA code<br><li>lastExtractionDate: last extraction timestamp used when `dateFrom` is not provided<br><li>debtPositionTypeOrgCodes: debt position type organization codes to include (nullable list)<br><li>skipDebtPositionTypeOrgCodesFilter: `true` to skip the `debtPositionTypeOrgCodes` filter<br><li>dateFrom: extraction start timestamp (nullable)<br><li>dateTo: extraction end timestamp (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0) | `ASSESSMENTS_REGISTRY` extractor from `mygov_anagrafica_uff_cap_acc` joined to `mygov_ente`. It supports optional debt position type organization code and modification-date filters and is paginated with `LIMIT/OFFSET`. |
