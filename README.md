...
| `GPD_ENABLED` | Enable the GPD filter for open debt positions, selecting only records with a GPD IUPD and status `S` | `false` |
...
| debt_positions (open/updated)                | `/db/mypay/debt-positions/debt-positions-open.sql` | <li>codIpaEnte: organization IPA code (required)<br><li>skipGpdEnabledFilter: `true` to skip the GPD filter; when `false`, only records with a non-null GPD IUPD and status `S` are selected<br><li>skipCodIuvFilter: `true` to skip the `iuvs` filter<br><li>iuvs: debt position IUV filter on `mygov_dovuto.cod_iuv` (nullable list)<br><li>skipDateFromFilter: `true` to skip the `dateFrom` filter<br><li>dateFrom: extraction start date at 00:00 applied to `mygov_dovuto.dt_ultima_modifica` (nullable)<br><li>skipDateToExclusiveFilter: `true` to skip the `dateToExclusive` filter<br><li>dateToExclusive: extraction end date +1 day at 00:00 applied to `mygov_dovuto.dt_ultima_modifica` (nullable)<br><li>limit: page size (> 0)<br><li>offset: page start (>= 0, nullable) | `DEBT_POSITIONS` extractor for open or modified debt positions from `mygov_dovuto`, joined to `mygov_flusso` and `mygov_ente`, with optional GPD, IUV, and update date interval filters. |
